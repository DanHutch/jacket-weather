{
  ...
  let favDrop = document.querySelector('.fav-bar-container')
  ...
  class WeatherGetter {
    ...
    getFavorites() {
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          let favoritesResponse = JSON.parse(xhr.response)["data"];
          this.renderFavorites(favoritesResponse)
        }
      };
    ...
    renderFavorites(responseData) {
      let favLocations = responseData.map(function (fav) {
        let location = fav.attributes.location
        return `
